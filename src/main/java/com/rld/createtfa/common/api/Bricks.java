...
public class Bricks {
  ...
  public static ArrayList<BlockBuilder<Block,?>> buildBlock (CreateTFARegistrate reg, String color) {
    String name;
  ...
  public static ArrayList<BlockBuilder<SlabBlock,?>> buildSlab (CreateTFARegistrate reg, String color) {
    String name;
  ...
  public static ArrayList<BlockBuilder<WallBlock,?>> buildWall (CreateTFARegistrate reg, String color) {
    String name;
    ...
    for (String prefix : TYPES) {
      ...
      CAPITALS.get(TYPES.indexOf(prefix))
          ...
          .item().tag(ItemTags.WALLS).model(BlockStateGenerator::brickWallItem).build()
      );
    }
    return ret;
  }
  ...
  public static <T extends Block> void recipeCraftingMossy (
    ...
    ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, ctx.get())
      .requires((dye == null) ? Blocks.BRICKS : BlockRegistry.BRICKS.get(dye).get(original))
